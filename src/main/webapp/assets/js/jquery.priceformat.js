	...
	$.fn.priceFormat = function(options) {
		...
		return this.each(function() {
			...
			function price_format(str, ignore) {
				...
				var centsVal = formatted.substr(formatted.length - centsLimit,
						centsLimit);
				...
				if (thousandsSeparator || $.trim(thousandsSeparator) != "") {
					...
					formatted = (centsLimit == 0) ? thousandsFormatted
			...
			function key_check(e) {
				...
				if (window.ctrl_down || metaKey) {
					if (code == 86)
						functional = true; // v: paste
					if (code == 67)
						functional = true; // c: copy
					if (code == 88)
						functional = true; // x: cut
					if (code == 82)
						functional = true; // r: reload
					if (code == 84)
						functional = true; // t: new tab
					if (code == 76)
						functional = true; // l: URL bar
					if (code == 87)
						functional = true; // w: close window/tab
					if (code == 81)
						functional = true; // q: quit
					if (code == 78)
						functional = true; // n: new window/tab
					if (code == 65)
						functional = true; // a: select all
				}
